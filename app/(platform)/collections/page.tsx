...
export default async function CollectionsPage() {
  ...
  const summary = await computeCollectionsSummary({});
  const todaysActions = await listCollectionActionsDueToday({});
  const cases = await listCollectionCases({});
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-neutral-600">
            Automatic collection cases are created when invoices become overdue with outstanding balances. Use this dashboard to chase and control receivables.
          </p>
        </div>
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        <div className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm lg:col-span-1">
          ...
          <div className="mt-4 grid gap-3">
            ...
            <SeverityRow label="High" value={summary.severityCounts.HIGH ?? 0} tone="high" />
            <SeverityRow label="Medium" value={summary.severityCounts.MEDIUM ?? 0} tone="medium" />
            <SeverityRow label="Low" value={summary.severityCounts.LOW ?? 0} tone="low" />
          </div>
          <div className="mt-5 rounded-xl border border-neutral-200 bg-neutral-50 p-4">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-neutral-500">Pending Actions Today</p>
            <p className="mt-2 text-2xl font-semibold text-neutral-950 tabular-nums">{summary.pendingActionsDueToday}</p>
            <p className="mt-1 text-xs text-neutral-600">Includes email/WhatsApp/manual actions scheduled for today.</p>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-neutral-200 bg-white shadow-sm lg:col-span-2">
          <div className="border-b border-neutral-200 px-6 py-4">
            ...
            <p className="mt-1 text-sm text-neutral-600">
              Actions scheduled for today across all projects. Automated sending runs via the cron endpoint; operators can still action cases manually.
            </p>
          </div>
                  ...
                  <tr key={c.id} className="border-t border-neutral-200">
                    ...
                    <td className="px-4 py-4 text-neutral-900">
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs text-neutral-500">{c.project.projectCode ?? c.project.id.slice(0, 8)}</span>
                      </div>
                    </td>
...
function SeverityRow(props: { label: string; value: number; tone: "critical" | "high" | "medium" | "low" }) {
  const tone =
    props.tone === "critical"
