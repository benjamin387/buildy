...
export async function PUT(
  ...
  try {
    ...
    const quotation = await prisma.quotation.findUnique({
      ...
      include: { project: { include: { commercialProfile: true } } },
    });
    ...
    const mustRevise =
      quotation.status === "APPROVED" || quotation.isLatest === false;

    const updatedOrNew = await prisma.$transaction(async (tx) => {
      if (mustRevise) {
        const quotationNumber = quotation.quotationNumber;
        const lastForNumber = await tx.quotation.findFirst({
          where: { projectId, quotationNumber },
          select: { version: true },
          orderBy: [{ version: "desc" }, { createdAt: "desc" }],
        });
        const nextVersion = (lastForNumber?.version ?? quotation.version) + 1;
        ...
        return await tx.quotation.create({
          data: {
            ...
            companyNameSnapshot: quotation.companyNameSnapshot,
            contactPersonSnapshot: quotation.contactPersonSnapshot,
            contactPhoneSnapshot: quotation.contactPhoneSnapshot,
            ...
            projectAddress2: quotation.projectAddress2,
            projectPostalCode: quotation.projectPostalCode,
            propertyType: quotation.propertyType,
            unitSizeSqft: quotation.unitSizeSqft,
          ...
          select: { id: true, version: true, quotationNumber: true },
        });
      }

      // Replace sections + items as one atomic update.
      await tx.quotationSection.deleteMany({ where: { quotationId } });
      await tx.quotationPaymentTerm.deleteMany({ where: { quotationId } });
      ...
      return { id: quotationId, quotationNumber: quotation.quotationNumber, version: quotation.version };
    });
    ...
    await createRevision({
      entityType: "Quotation",
      entityId: mustRevise ? updatedOrNew.id : quotationId,
      projectId,
      actorUserId: userId,
      note: mustRevise ? "Revised quotation" : "Updated quotation",
      data: { quotationId, computed, revised: mustRevise, newQuotationId: updatedOrNew.id },
    });

    return NextResponse.json({ success: true, data: { id: updatedOrNew.id } });
  } catch (error) {
